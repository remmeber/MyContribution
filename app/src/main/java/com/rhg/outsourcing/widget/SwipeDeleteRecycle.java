...
public class SwipeDeleteRecycle extends RecyclerView {
    ...
    public SwipeDeleteRecycle(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public SwipeDeleteRecycle(Context context, AttributeSet attrs, int defStyle) {
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN: {
                ...
                if (scrollState == SCROLL_ON) {
                    scrollState = SCROLL_OFF;
    ...
    private void scrollByDistanceX() {
        ...
        if (orientation == Orientation.VERTICAL) {
            if (itemView.getScrollX() >= screenWidth / 8) {
                scrollLeft();
